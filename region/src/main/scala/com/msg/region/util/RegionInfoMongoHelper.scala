            ...
            if (entityObj != null && entityObj.keySet().size() > 0) {
              ...
              while (iterator.hasNext()) {
                ...
                if (status == 1) {
                  ...
                  val topicInfo = TopicInfo(topic, code, key, isStore, storeMethod, sendNum, storeNum, isRegx, status,
                    isTemplate, isNotify, regxParam, defaultTemp, defaultNotify, broadStatus)
