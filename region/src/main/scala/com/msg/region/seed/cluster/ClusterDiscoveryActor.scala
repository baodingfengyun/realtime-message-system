...
import com.msg.region.seed.etcd.EtcdError
import com.msg.region.seed.etcd.EtcdException
import com.msg.region.seed.etcd.EtcdNode
import com.msg.region.seed.etcd.EtcdResponse
...
class ClusterDiscoveryActor(
	...
	when(Leader) {
		...
		case Event(MemberUp(member), seeds) ⇒
			RegionInfoMongoHelper.saveRegion(Constants.REGION_INFO, member.address.toString + "/user/exchange/receivedata", 1)
		...
		case Event(MemberExited(member), seeds) ⇒
			RegionInfoMongoHelper.deleteRegion(Constants.REGION_INFO, member.address.toString + "/user/exchange/receivedata")
	...
	onTransition {
		case (_, Follower) ⇒
			...
			cluster.subscribe(self, initialStateMode = InitialStateAsSnapshot, classOf[MemberEvent])
			setTimer("seedsFetch", SeedsFetchTimeout, settings.seedsFetchTimeout, true)
			fetchSeeds()
	}
	...
	when(Follower) {
		...
		case Event(SeedsFetchTimeout, _) ⇒
			...
			goto(Election)
		case Event(LeaderFetchTimeout, _) =>
			log.info(s"failed to fetch header node information in ${settings.seedsFetchTimeout.toMillis} ms")
			goto(Election)
...
object ClusterDiscoveryActor {
   ...
   */
	...
	private case object LeaderFetchTimeout
